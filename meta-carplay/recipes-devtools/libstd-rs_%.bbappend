...
# Rust's libstd vendors libc 0.2.x.  Use the same pinned libc alpha as Catplay
# so the target standard library also gets the current riscv32/musl ABI
# definitions.  Do not key this on PV: libstd-rs is provided by oe-core and its
# version advances with the layer.  Patch or manifest drift must fail loudly
# instead of silently disabling the workaround.
LIBC_ALPHA_VERSION = "1.0.0-alpha.4"
LIBC_ALPHA_SRCREV = "a1fcda25057b609984288ba2a280011367a493c1"
LIBC_ALPHA_ENABLED = "${@'1' if d.getVar('TARGET_ARCH') == 'riscv32' and d.getVar('TCLIBC') == 'musl' else '0'}"

SRC_URI:append = "${@' git://github.com/rust-lang/libc.git;protocol=https;nobranch=1;name=libc;destsuffix=libc-alpha file://0001-libstd-riscv32-musl-libc-alpha-compat.patch;patchdir=../..' if d.getVar('LIBC_ALPHA_ENABLED') == '1' else ''}"
SRCREV_libc = "${LIBC_ALPHA_SRCREV}"
...
python prepare_libc_alpha () {
    ...
    libc_src = pathlib.Path(d.getVar("UNPACKDIR")) / "libc-alpha"
    ...
    old_dependency = r'(?m)^libc = \{ version = "0\.2\.\d+", default-features = false, features = \[$'
    new_dependency = 'libc = { path = "../libc-alpha", default-features = false, features = ['
    manifest_text, dependency_replacements = re.subn(old_dependency, new_dependency, manifest_text)
    if dependency_replacements == 1:
        std_manifest.write_text(manifest_text)
    elif dependency_replacements != 0 or manifest_text.count(new_dependency) != 1:
        bb.fatal(
            "libstd-rs %s: expected exactly one libc 0.2.x dependency in %s"
            % (d.getVar("PV"), std_manifest)
        )
