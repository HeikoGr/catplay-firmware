...
# Identify which state of this tree a build came from. This ends up in the
# kernel banner, i.e. the "Linux version ..." line at the top of dmesg, so a
# captured log says what it was built from. Derived from git rather than from
# the wall clock on purpose: the value only changes when the tree changes, so
# it does not force a kernel rebuild on every invocation.
fw_version="$(git -C "${root_dir}" describe --tags --always --dirty 2>/dev/null || echo unknown)"
fw_date="$(git -C "${root_dir}" log -1 --format=%cd --date=format:%Y-%m-%d 2>/dev/null || echo unknown)"
echo "[*] Firmware version: ${fw_version} (${fw_date})"
